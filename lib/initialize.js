...
window.onload = function() {
    // Display the counting of all songs
    $("#songCount").html(`You currently have ${data_music_list.length} songs.`);
    ...
    for (let i = 0; i < data_music_list.length; i++) {
        ...
        html += `
            <div onclick="playMusic(${i})" class="music_data_list">
                <div class="music_data_numbering_con">
                    <div class="music_data_numbering">${i+1}. </div>
                </div>
...
function searchFilterMusicTitle() {
  // Get search input and convert to uppercase only once
  const searchFilter = $("#searchInput").val().toUpperCase();

  // Select music items directly, cache the selection
  const $musicItems = $(".music_data_list");

  if ($("#searchInput").val() == null || $("#searchInput").val() == "") {
    $("#resetInputButton").hide();
  }
  else {
    $("#resetInputButton").show();
  }

  // Iterate through music items
  $musicItems.each(function() {
    const $title = $(this).find(".music_data_title"); // Cache title element
    const titleText = $title.text().toUpperCase();
    

    // Use toggle for simpler show/hide logic
    $(this).toggle(titleText.indexOf(searchFilter) > -1);
  });
}
...
function playTheMusicByIndexNumber(indexNumber) {
    ...
    $("#playMusicButton").attr("title", "Pause");
...
function playMusic(index) {
    ...
    audioInterval = setInterval(function() {
        ...
        if (progress.value == progress.max) {
            // Determine next action based on shuffle, loop settings, etc.
...
function playTheCurrentMusic() {
    ...
    if (isPlaying) {
        ...
        $("#playMusicButton").attr("title", "Play");
    }
    else {
        isPlaying = true;
        isPause = false;
...
function getLoopMusic() {
    ...
    switch(loopCounter) {
        case 1:
            ...
            $("#loopButton").attr("title", "Loop All Music");
        break;
        case 2:
            ...
            $("#loopButton").attr("title", "Loop Current Music");
        break;
        case 3:
...
function changeSeekBarBGcolor() {
    ...
    activeColorMusicList = `${seekBarColors[randomColor]}`;
}
// Callback
